...
def new_plugin_from_config(config: Config, type: Type,
    ...
    plugin_dict = config_plugin.get(key, exception_if_missing=True)
    return new_plugin_from_dict(
        config=config, type=type, plugin_dict=plugin_dict)
...
def new_plugin_from_dict(config: Config, type: Type,
                         plugin_dict: Dict[str, Any]) -> UCTTPlugin:
    ...
    """
    if not UCTT_PLUGIN_CONFIG_KEY_PLUGINID in plugin_dict:
        raise ValueError(
            "Plugin dict was missing plugin_id : {}".format(plugin_dict))
    plugin_id = plugin_dict[UCTT_PLUGIN_CONFIG_KEY_PLUGINID]
    if not UCTT_PLUGIN_CONFIG_KEY_INSTANCEID in plugin_dict:
        instance_id = '{}-{}'.format(type, plugin_id)
    else:
        instance_id = plugin_dict[UCTT_PLUGIN_CONFIG_KEY_INSTANCEID]

    instance_config = config
    # If we were given a output config, then create a copy of the config
    # object and add the passed config as new sources to the copy
    if UCTT_PLUGIN_CONFIG_KEY_CONFIG in plugin_dict:
        instance_config = config.copy()
        # Add a dict source plugin for the passed 'config', giving it source id
        # just to make identification easier
        instance_config.add_source(
            PLUGIN_ID_SOURCE_DICT, 'output-{}-{}'.format(
                plugin_id, instance_id)).set_data(
    ...
    if UCTT_PLUGIN_CONFIG_KEY_ARGS in plugin_dict:
        plugin.arguments(**plugin_dict[UCTT_PLUGIN_CONFIG_KEY_ARGS])
