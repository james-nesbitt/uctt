...
from uctt import new_clients_from_dict
from uctt.provisioner import ProvisionerBase

logger = logging.getLogger('uctt.contrib.terraform.provisioner')

UCTT_TERRAFORM_PROVISIONER_CONFIG_LABEL = 'terraform'
""" config label loading the terraform config """
UCTT_TERRAFORM_PROVISIONER_CONFIG_PLAN_PATH_KEY = 'plan.path'
""" config key for the terraform plan path """
UCTT_TERRAFORM_PROVISIONER_CONFIG_STATE_PATH_KEY = 'state.path'
""" config key for the terraform state path """
UCTT_TERRAFORM_PROVISIONER_CONFIG_VARS_KEY = 'vars'
""" config key for the terraform vars Dict, which will be written to a file """
UCTT_TERRAFORM_PROVISIONER_CONFIG_VARS_PATH_KEY = 'vars_path'
""" config key for the terraform vars file path, where the plugin will write to """
UCTT_TERRAFORM_PROVISIONER_CONFIG_PATH_KEY = ''
""" config key for the terraform plan path """


UCTT_TERRAFORM_PROVISIONER_DEFAULT_VARS_FILE = 'mtt_terraform.tfvars.json'
""" Default vars file if none was specified """
UCTT_TERRAFORM_PROVISIONER_DEFAULT_STATE_SUBPATH = 'mtt-state'
""" Default vars file if none was specified """

UCTT_TERRAFORM_OUTPUT_KEY_CLIENTS = 'clients'
...
class TerraformProvisionerPlugin(ProvisionerBase):
    ...
    def prepare(self, label:str=UCTT_TERRAFORM_PROVISIONER_CONFIG_LABEL):
        """
        ...
        self.working_dir = self.terraform_config.get(UCTT_TERRAFORM_PROVISIONER_CONFIG_PLAN_PATH_KEY)
        """ all subprocess commands for terraform will be run in this path """

        state_path = self.terraform_config.get(UCTT_TERRAFORM_PROVISIONER_CONFIG_STATE_PATH_KEY, exception_if_missing=False)
        """ terraform state path """
        if not state_path:
            state_path = os.path.join(self.working_dir, UCTT_TERRAFORM_PROVISIONER_DEFAULT_STATE_SUBPATH)

        self.vars = self.terraform_config.get(UCTT_TERRAFORM_PROVISIONER_CONFIG_VARS_KEY)
        """ List of vars to pass to terraform.  Will be written to a file """
        if not self.vars:
            self.vars = {}

        vars_path = self.terraform_config.get(UCTT_TERRAFORM_PROVISIONER_CONFIG_VARS_PATH_KEY, exception_if_missing=False)
        """ vars file containing vars which will be written before running terraform """
        if not vars_path:
            vars_path = os.path.join(self.working_dir, UCTT_TERRAFORM_PROVISIONER_DEFAULT_VARS_FILE)
    ...
    def get_client(self, plugin_id:str='', name:str=''):
        """ Retrieve a client as defined by TF outputs """
        try:
            client_output = self.get_output(UCTT_TERRAFORM_OUTPUT_KEY_CLIENTS)
        except Exception as e:
            raise Exception("Terraform plan has no clients output") from e
