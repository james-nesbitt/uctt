...
logger = logging.getLogger('uctt.cli.ansible')
    ...
    def _select_provisioner(self, instance_id: str = ''):
        ...
        if instance_id:
            return self.environment.fixtures.get_fixture(
                type=Type.PROVISIONER, plugin_id='uctt_ansible', instance_id=instance_id)
        else:
            # Get the highest priority provisioner
            return self.environment.fixtures.get_fixture(
                type=Type.PROVISIONER, plugin_id='uctt_ansible')
